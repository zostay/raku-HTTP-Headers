...
=SUBTITLE Tools for working with HTTP message headers
...
Constructs a new object for working with headers.

=head2 method header

    my $content-type = $headers.header(Content-Type);
    $header.header(Content-Type) = "text/html";
    my $x-custom = $headers.header("X-Custom-Header");
    $headers.header("X-Custom-Header") = "foo";

This method is writable and allows the use of either the values in the C<HTTP::Header::Standard::Name> enumeration or string values. In general, you should not use strings when you can use the enumeration in your code. By using the enumeration, you can find typos and other errors more quickly.
...
You may also set headers with L<DateTime>, L<Instant>, and L<Duration> objects and it should do the right thing. For example,
...
When you read a header, the value returns is a L<HTTP::Header> object.
...
See L<HTTP::Header> for details.

=head2 method remove-header

    $headers.remove-header(Content-Type);
    $headers.remove-header("X-Foo-Custom");

This method will remove a header from the list. The removed L<HTTP::Header> object is returned.

=head2 method remove-headers

    $headers.remove-headers(Content-Type, "X-Foo-Custom");

This method remove multiple headers. The removed header objects are returned in a list.

=head2 method remove-content-headers

    $headers.remove-content-headers;
...
This calls L</method as-string> with no arguments.

=head2 method for-PSGI
...
The following methods are provided as a shortcut for L</method header> and can be used as an accessor or mutator.
